...
const BoatFrom: FC<TripeFormProps> = ({ onClose }) => {
  const [boat, setBoat] = useState<Boat>(boatVoid);
        ...
        <form className="flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-2">
            ...
            >
              ...
              <SelectContent>
                {capitaine.map((c) => (
                  <SelectItem key={c.id} value={c.id}>{`${c.name} ${c.lastName}`}</SelectItem>
                ))}
              </SelectContent>
            </Select>
